...
const UserBadge = () => {
    ...
    return (
        <>
            <div ref={divRef}
                 className=' flex items-center gap-x-4 md:shadow-lg hover:bg-gray-100 md:bg-light-gray cursor-pointer dark:bg-dark-green md:dark:bg-deep-green hover:dark:bg-dark-green px-5 py-3 rounded-lg'
                 onClick={handleClick}>
                {userInfo ?
                    <>
                        <img src={userInfo.iconUrl} style={{width: "40px", height: "40px"}}
                             className="rounded-4xl border-green-400 border-2" alt=""/>
                        <div>
                            <p className='text-black dark:text-white font-semibold '>{userInfo.firstAndLastName}</p>
                            <p className='text-black dark:text-white font-light text-sm'>{pascalCaseWord(userInfo.role)}</p>
                        </div>
                        {user.role == USER_ROLES.USER ?
                            <Badge color="error" variant="dot"
                                   invisible={annoucements ? !annoucements.some(annoucement => annoucement.seen == false) : true}>
                                <NotificationsIcon/>
                            </Badge>
                            : null}
                    </>
